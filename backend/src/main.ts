...
// Helper function to convert JSON to YAML format
// Simple conversion without external dependencies for Swagger YAML endpoint
  ...
  if (Array.isArray(obj)) {
    obj.forEach((item) => {
      if (typeof item === 'object' && item !== null && !Array.isArray(item)) {
        yaml += `${indentStr}- `;
        const itemYaml = convertJsonToYaml(item, indent + 1);
    ...
    yaml += `${indentStr}${formatYamlValue(obj)}\n`;
  }
...
Demon Slayer Corps Account:
  ...
  * [Chat](#/Chat%20controller) — operations with chat
  ...
  `
    ...
    .setVersion('1.0')
    .addServer(process.env.URL || 'http://localhost:3000')
    .build();
  const document = SwaggerModule.createDocument(app, options);

  // Configure Swagger with proper server URL from environment
  // This ensures Swagger UI works correctly on both localhost and server deployments
  const swaggerOptions = {
    swaggerOptions: {
      persistAuthorization: true,
      // Use the URL from environment or default to localhost
    ...
    customSiteTitle: 'Demon Slayers API Documentation',
    customfavIcon: '/assets/img/favicons/demon-slayer-logo.jpg',
    customCss: '.swagger-ui .topbar { display: none }'
  };

  SwaggerModule.setup('swagger', app, document, swaggerOptions);

  // Add /swagger.json endpoint to serve the Swagger JSON
  // This endpoint provides the OpenAPI specification in JSON format
  ...
  // NOTE:
  // Nest's SwaggerModule.setup('swagger', ...) already registers:
  // - /swagger-json
  // - /swagger-yaml
  // Adding our own /swagger-json handler causes Fastify's FST_ERR_DUPLICATED_ROUTE.
  // We keep /swagger.json (dot) for compatibility, and rely on Nest for /swagger-json.

  // Add /swagger.yaml endpoint to serve the Swagger YAML
  // Note: For YAML support, js-yaml package would be needed
  // For now, we'll serve JSON with YAML content-type header if requested
  server.get('/swagger.yaml', async (request, reply) => {
    reply.header('Content-Type', 'text/yaml; charset=utf-8');
    reply.header('Access-Control-Allow-Origin', '*');
    // Convert JSON to YAML format (simple conversion)
    // In production, consider using js-yaml package for proper conversion
    const yamlString = convertJsonToYaml(document);
    reply.send(yamlString);
  });
