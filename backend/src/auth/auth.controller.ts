...
export class AuthController {
  ...
  async loginWithRSAJwtKeys(
    ...
    if (req.op === FormMode.OIDC) {
      throw new BadRequestException(
        'OIDC login is disabled. Keycloak has been removed from this deployment.'
      );
    }
  ...
  async getOidcClient(): Promise<never> {
    throw new ServiceUnavailableException(
      'Keycloak is not configured. OIDC client endpoints have been removed.'
    );
  }
